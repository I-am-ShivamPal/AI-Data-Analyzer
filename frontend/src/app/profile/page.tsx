  ...
  const inner = (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 pb-4 border-t border-white/[0.06] pt-3">
        ...
        <button onClick={logout} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors group">
        ...
        <div className="relative">
          ...
          {userOpen && (
            <div className="absolute right-0 top-full mt-2 w-44 bg-[#0B1120] border border-white/10 rounded-xl shadow-xl z-50 overflow-hidden py-1">
              <button onClick={logout} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-400 hover:bg-red-500/10"><LogOut className="w-4 h-4" />Logout</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-br from-[#0B1120] to-[#070B14] border border-white/10 rounded-3xl p-6 sm:p-10">
      ...
      <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
        ...
        <div className="flex-1">
          <h2 className="text-2xl sm:text-3xl font-bold text-white">{user?.name || "User"}</h2>
          <p className="text-slate-400 text-sm mt-1 capitalize">{user?.role || "Normal"} User <span className="mx-2 text-white/20">•</span> Member since {user?.created_at ? new Date(user.created_at).toLocaleDateString() : "2026"}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="px-3 py-2 bg-white/[0.02] border border-white/5 rounded-lg opacity-70">
              <p className="text-sm font-medium text-slate-300 capitalize">{user?.role || "Normal"} User</p>
            </div>
